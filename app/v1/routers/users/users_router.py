...
from app.v1.schemas.user.auth import SignUpRequest,SignInRequest,ResendOtpRequest,ForgotPasswordRequest,ValidateOtpRequest,ResetPasswordRequest
...
async def forgot_password(forgot_password_request: ForgotPasswordRequest, user_manager: UserManager = Depends(get_user_manager)):
...
async def reset_password(reset_password_request: ResetPasswordRequest, user_manager: UserManager = Depends(get_user_manager)):
    validation_result = reset_password_request.validate()
    if validation_result:
        return validation_result
    try:
        # Reset password logic (verify OTP and update password)
        await user_manager.reset_password(reset_password_request.email, reset_password_request.phone, reset_password_request.password)
...
async def validate_otp(validate_otp_request: ValidateOtpRequest, user_manager: UserManager = Depends(get_user_manager)):
    ...
    try:
        # Validate OTP logic
        result = await user_manager.validate_otp(email=validate_otp_request.email,phone=validate_otp_request.phone, otp=validate_otp_request.otp)
...
async def update_profile(user_id: str, update_profile_request: User, user_manager: UserManager = Depends(get_user_manager)):
    try:
