...
from app.v1.schemas.category.category import CreateCategoryRequest,AllCategoryRequest,GetCategoryRequest,UpdateCategoryRequest,DeleteCategoryRequest

router = APIRouter()


@router.post("/create-category", status_code=status.HTTP_200_OK)
async def create_category(category_request: CreateCategoryRequest, category_manager:CategoryManager  = Depends(get_category_manager)):
...
async def category_list(page: int = Query(1, ge=1, description="Page number (must be >= 1)"),
    ...
    #     return validation_result
    try:
        result = await category_manager.category_list(page,limit,search)
...
async def get_category(
    ...
    try:
        ...
        result = await category_manager.get_category_by_id(id)
...
async def update_category(
    ...
    if not (category_request.name or category_request.status):
        raise HTTPException(
            ...
            detail="At least one field (name or status) must be provided"
        )

    try:
        # Call the CategoryManager to update the category by id
        result = await category_manager.update_category_by_id(id, category_request)
...
async def delete_category(
	...
	try:
		...
		result = await category_manager.delete_category_by_id(id)
